module DrilldownHelper
  ...
  def caption
    result = @search.title ? @search.title : "Transaction #{t(@search.select_value.downcase)}" + (@dimensions && @dimensions[0] && @dimensions[0][:pretty_name] ? " by #{@dimensions[0][:pretty_name]}" : "")
  ...
  def excel_summary_row(result, parent_result = nil, dimension = 0, headers = [])
    ...
    if result[:rows]
      ...
      if @search.list
        xml << render(:partial => '/drilldown/excel_transaction_list', :locals => {:result => result})
      end
    end
